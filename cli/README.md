# oc - opencode proxy switcher CLI

用 [scriptc](https://github.com/vercel-labs/scriptc) 编译的原生二进制（当前发布 macOS arm64），管理 opencode 代理端点并改写本地的 opencode 配置。端点列表存储在本地 `~/.oc/config.json`，不存任何凭据。

## 安装

### 方式一：下载预编译二进制（推荐，macOS arm64）
...
oc use NAME           # 切换端点（自动更新本地 opencode 配置）
oc del NAME           # 删除端点
...
| 发现位置 | 文件 |
|----------|------|
| 全局 | `~/.config/opencode/opencode.json(c)`、`~/.opencode/opencode.json(c)` |
| 项目级 | 从 cwd 向上逐级发现的 `opencode.json(c)` 与 `.opencode/opencode.json(c)` |

细节：

- OpenCode 配置按 opencode 自身加载优先级处理：全局目录同时存在 `opencode.jsonc` 和 `opencode.json` 时更新 jsonc（后加载、优先级更高）。
- 支持带注释和尾逗号的 jsonc（`//`、`/* */`），解析时自动剥离。
- 如果 opencode 未安装且无任何配置，自动创建 `~/.config/opencode/opencode.jsonc`。
